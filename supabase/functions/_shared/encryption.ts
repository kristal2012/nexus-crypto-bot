...
const ENCRYPTION_KEY = Deno.env.get('CHAVE_CRIPTO_BINANCE_SEGURA_2025_XYZ');
const PBKDF2_ITERATIONS = 100000; // 100k iterations for security

// Validate encryption key on module load
if (!ENCRYPTION_KEY) {
  throw new Error('CHAVE_CRIPTO_BINANCE_SEGURA_2025_XYZ not configured');
}
if (ENCRYPTION_KEY.length < 32) {
  throw new Error('CHAVE_CRIPTO_BINANCE_SEGURA_2025_XYZ must be at least 32 characters');
}
// Verify character diversity to prevent weak keys
const uniqueChars = new Set(ENCRYPTION_KEY).size;
if (uniqueChars < 16) {
  throw new Error('CHAVE_CRIPTO_BINANCE_SEGURA_2025_XYZ must contain at least 16 unique characters for sufficient entropy. Generate with: openssl rand -base64 48');
}

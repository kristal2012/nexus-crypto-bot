-- Drop and recreate the acquire_analysis_lock function with better error handling
-- and more reasonable cooldown period

DROP FUNCTION IF EXISTS public.acquire_analysis_lock(uuid, integer);

CREATE OR REPLACE FUNCTION public.acquire_analysis_lock(
  p_user_id uuid, 
  p_cooldown_minutes integer DEFAULT 2  -- Reduced from 15 to 2 minutes
)
RETURNS TABLE(
  is_active boolean,
...
BEGIN
  ...
  -- Check rate limiting with more lenient policy
  IF v_config.last_analysis_at IS NOT NULL THEN
    ...
    IF v_time_since_last_analysis < v_cooldown_seconds THEN
      RAISE EXCEPTION 'Rate limit: % seconds remaining. Please wait before running another analysis.', 
